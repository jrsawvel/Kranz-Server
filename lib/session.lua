...
function _send_login_link(email_rcpt, digest, date_time)
    ...
    local message = "Copy code to log into the site.\n\n" .. digest .. "\n"

    -- need to check for error and handle it.
...
function M.create_and_send_no_password_login_link(hash_ref_login)
    ...
    if user_submitted_email == nil or user_submitted_email == "" then
        return rj.report_error("400", "Invalid input.", "Insufficient data was submitted.")
    else
        ...
        if user_submitted_email ~= author_email and user_submitted_email ~= backup_author_email then
            return rj.report_error("400", "Invalid input.", "Data was not found.")
        else
            digest = _create_session_id() -- return the login digest to be emailed

            -- todo : add error checking to the _send_login_link function and check 
            --        for error here.
            _send_login_link(author_email, digest, date_time)
            _send_login_link(backup_author_email, digest, date_time)
...
function M.activate_no_password_login(hash)

    local rev = hash.rev -- the random_string created above and sent to the author

    local h_session = _read_session_file(rev)

    if h_session.iserror then
        return rj.report_error("400", h_session.msg1, h_session.msg2)
    else
        if h_session.status ~= "pending" then
            return rj.report_error("400", "Unable to login.", "Invalid session information submitted.")
        else
...
function M.logout(hash)
    ...
    if config_author_name ~= author_name then
        return rj.report_error("400", "Unable to logout.", "Invalid info submitted.")
    else
        local h_session = _read_session_file(rev)
